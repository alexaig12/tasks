...
export function isCorrect(question: Question, answer: string): boolean {
    const hairCutAnswer = answer.trim().toLowerCase();
    const hairCutExpect = question.expected.trim().toLowerCase();

    if (hairCutAnswer === hairCutExpect) {
        return true;
    } else {
        return false;
    }
...
export function toMarkdown(question: Question): string {
    ...
    // Add the name line
    format.push(`# ${question.name}`);

    // Add the body line
    format.push(question.body);
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newQuestion: Question = {
        ...
        options: oldQuestion.options ? [...oldQuestion.options] : [],
...
export function addOption(question: Question, newOption: string): Question {
    // Create a new question object with the same properties as the original
    const updatedQuestion: Question = {
        ...question,
        options: [...question.options, newOption] // Add the new option to a new copy of the options array
    };
